...
API_URL = 'https://pulsetrack-back.onrender.com/api/v1'

truck_data = {
    'fleet_id': str(uuid.uuid4()),
    'truck_identifier': 'TEST-PROD-TRUCK',
    'plate': 'TEST-PROD-' + str(int(datetime.now().timestamp())),
    'make': 'TestMake',
    'model': 'TestModel',
    'year': 2024,
...
print('📤 Sending truck creation request to PRODUCTION backend:', f"{API_URL}/trucks/")
...
try:
    ...
    if hasattr(error, 'response') and error.response is not None:
        ...
        try:
            ...
            print('  Response Text:', error.response.text[:500])
